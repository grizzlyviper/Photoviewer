...
function listAlbums() {
  ...
  //////////////
    ...
    data.Contents.forEach(e => {
      ...
      e.displayName = key.replace(/[-_]/g, " ") //+ " " + e.date;
  ...
  //////////////
    ...
    const photos = Object.values(groups).map(group => {
      const photo = group.at(-1)
      ...
      return getHtml([
        `<div class="card">
          <div>
            <img style="width:25vw;" src="${photoUrl}"/>
          </div>
          <div>
          ${photo.displayName}
          </div>
          <div>
          ${photo.date.toString().slice(0,25)}
          </div>
        </div>
        `,
      ]);
    ...
    document.getElementsByTagName('img')[0].setAttribute('style', 'width:25vw;');
  });
}
listAlbums();
